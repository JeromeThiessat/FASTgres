...
class Encoder:
    ...
    def encode_query(self, encoding_dict: dict):
        encoded_query = list()
        merged_context = sorted(set().union(*self.context.covered_contexts))
        logger = get_logger()
        logger.info(f"Using merged context {merged_context} for encoding.")
        for table in sorted(merged_context):
            if table not in encoding_dict.keys():
                # extend by all column encodings to 0
                encoded_query.extend([0] * 4 * len(self.encoding_info.db_type_dict[table].keys()))
                continue
